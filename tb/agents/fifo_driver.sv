...
function fifo_driver::new(string name, uvm_component parent);
  super.new(name, parent);
endfunction

function void fifo_driver::build_phase(uvm_build_phase phase);
  super.build_phase(phase);
  if(!uvm_config_db #(virtual fifo_if)::get(this, "", "vif", vif))
    `uvm_fatal("NOVIF", "Virtual interface not found")
endfunction

function void fifo_driver::end_of_elaboration_phase(uvm_end_of_elaboration_phase phase);
  super.end_of_elaboration_phase(phase);
  `uvm_info("DRIVER", "fifo_driver end_of_elaboration_phase", UVM_LOW)
endfunction

function void fifo_driver::start_of_simulation_phase(uvm_start_of_simulation_phase phase);
  super.start_of_simulation_phase(phase);
  `uvm_info("DRIVER", "fifo_driver start_of_simulation_phase", UVM_LOW)
endfunction

function void fifo_driver::extract_phase(uvm_extract_phase phase);
  super.extract_phase(phase);
  `uvm_info("DRIVER", "fifo_driver extract_phase", UVM_LOW)
endfunction

function void fifo_driver::check_phase(uvm_check_phase phase);
  super.check_phase(phase);
  `uvm_info("DRIVER", "fifo_driver check_phase", UVM_LOW)
endfunction

function void fifo_driver::report_phase(uvm_report_phase phase);
  super.report_phase(phase);
  `uvm_info("DRIVER", "fifo_driver report_phase", UVM_LOW)
endfunction

function void fifo_driver::final_phase(uvm_final_phase phase);
